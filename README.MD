...
account:{account_id}:equity -> string: equity
account:{account_id}:used_margin -> string: used_margin
```
### MarketRedisClient  
```
...
4. **Install and run:** This will make the endpoints available at http://localhost:8000 
```bash
poetry install
...
5. **Run integration test (optional):** This checklist validates the core functionality of the derivatives trading platform as specified in instructions.md. Each test verifies a key requirement: margin checks, trade execution, position tracking, P&L calculations, and liquidation detection.
```bash
...
│                 │    │                    │    │                   │
│ • Request/      │    │ • Calculations     │    │ • AccountRedis    │
│   Response      │    │   Service          │    │   Client          │
│ • Validation    │    │ • TradingService   │    │ • MarketRedis     │
│ • Error Handling│    │ • MarginService    │    │   Client          │
│                 │    │ • Pure Functions   │    │ • PostgresClient  │
└─────────────────┘    └────────────────────┘    └───────────────────┘
```
